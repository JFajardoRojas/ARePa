...
#input
atom2change = str('Si') #input("Which atom do you wanna change? (Atomic symbol): ")
atom_for = str('Al') #input("Which atom do you wanna place instead? (Atomic symbol): ")

list_zeolite = os.listdir("zeo_data")
#print(list_zeolite)

for zeolite in list_zeolite:
    #File reading
    zeo_file = open("zeo_2change/"+zeolite, "r")
    z_file = zeo_file.read()
    z_file_list = z_file.split()
    z_file_leo = z_file.split('\n')

    #Identification of position in te file
...
#print(coord_in, coord_fin, coord_len)
